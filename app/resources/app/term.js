...
window.addEventListener('load', function load(event) {
	...
	document.getElementById('btn_connect').onclick = function(event) {
		...
		if (connexion){
			...
			document.getElementById('btn_envoi').disabled=true
			s_p.close(function (err) { moniteur.innerHTML += '--- CLOSE SERIAL PORT ---<br>' })
			connexion = false
		} else {
			...
			s_p.open(function (err) { if(!err) moniteur.innerHTML += '--- OPEN SERIAL PORT ---<br>'; else moniteur.innerHTML += '--- ERROR OPENING SERIAL PORT: '+err.message+' ---<br>' })
			connexion = true
